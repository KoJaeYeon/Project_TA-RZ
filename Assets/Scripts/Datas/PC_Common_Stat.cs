public class PC_Common_Stat : Stat
{
    ...
    public PC_Common_Stat()
        : this(101, "PC_Type1", 20f, 100f, 6f, 50, 60f, 20f, 35f,5,2,1)
    {
    }

    public PC_Common_Stat(int id, string type, float atk_Power, float hp, float move_Speed, int resource_Own_Num, float stamina_Gain, float drain_Stamina, float dash_Stamina, float drain_MaxRange, float range_Speed, float pull_Speed)
    {
        Id = id;
        Type = type;
        Atk_Power = atk_Power;
    ...
    {
        return new PC_Common_Stat(Id, Type, Atk_Power, HP, Move_Speed, Resource_Own_Num, Stamina_Gain, Drain_Stamina, Dash_Stamina, Drain_MaxRange, Range_Speed, Pull_Speed);
    }
